...
## Codes for model development and analysis of the ANN model ##

# Creating lists to store prediction and statistical outputs from internal validation
ann_pred_test <- list()
ann_acc_test <- list()
ann_balancedacc_test <- list()
ann_precision_test <- list()
ann_sensitivity_test <- list()
ann_specificity_test <- list()
ann_npv_test <- list()
ann_roc_test <- list()
ann_cstat_test <- list()
ann_rsquared_test <- list()
ann_slope_test <- list()
n_imp <- n_distinct(data_raw$imp)

# Running a for-loop so that the model runs on each imputed dataset
ann_prog <- 0
ann_start <- Sys.time() # for tracking computation time
for (i in 1:n_imp){
  
  # Isolating the imputed datasets
  n <- sample(c(TRUE,FALSE), nrow(data_raw[data_raw$imp == i,]), replace = TRUE, prob = c(0.8,0.2))
  ...
  train <- cbind(data.frame(predict(dummy_train, newdata = df_smote_train)),fa_cat_end = df_smote_train$fa_cat_end)
  df_test <- data_raw[data_raw$imp == i,-c(27)][!n,]
  dummy_test <- dummyVars(" ~ . - imp - participant_id - wave - fa_cat_end", data = df_test)
  test <- cbind(data.frame(predict(dummy_test, newdata = df_test)),fa_cat_end = df_test$fa_cat_end)
 
  # Running the training functions
  ann_model <- nnet(fa_cat_end ~ ., data = train, size = 1, trace = FALSE) # fitting the ANN model on all variables
# ann_grid <- expand.grid(size = c(5:8), decay = c(0.01,0.05,0.1))
  ann_tuned <- train(fa_cat_end ~ ., data = train, method = "nnet", tuneLength = 10, trace = FALSE, trControl = trainControl(method = "repeatedcv", number = 10, search = "random")) # training tuned parameters
  ann_final <- nnet(fa_cat_end ~ ., data = train, trace = FALSE, size = ann_tuned$bestTune$size, decay = ann_tuned$bestTune$decay) # fitting the tuned hyperparameters into the final model

  # Conducting statistical analysis on the dataset with bootstrapping
  ann_acc_func <- function(data, index){ # creating a function for calculating the accuracy
    df <- data[index,]
    dum <- dummyVars(" ~ . - imp - participant_id - wave - fa_cat_end", data = df)
    dat <- cbind(data.frame(predict(dum, newdata = df)),fa_cat_end = df$fa_cat_end)
    mod <- ann_final
    pred <- predict(mod, dat, type = "raw")
  ...
  ann_acc <- boot::boot(data = df_test, statistic = ann_acc_func, R = 1000) # applying the bootstrapping function on the testing dataset
  ann_acc_test[[i]] <- ann_acc$t # storing the function results from each bootstrapped instance
  
  ann_balancedacc_func <- function(data, index){ # creating a function for calculating the balanced accuracy
    df <- data[index,]
    dum <- dummyVars(" ~ . - imp - participant_id - wave - fa_cat_end", data = df)
    dat <- cbind(data.frame(predict(dum, newdata = df)),fa_cat_end = df$fa_cat_end)
    mod <- ann_final
  ...
  ann_balancedacc <- boot::boot(data = df_test, statistic = ann_balancedacc_func, R = 1000) # applying the bootstrapping function on the testing dataset
  ann_balancedacc_test[[i]] <- ann_balancedacc$t # storing balanced accuracy values
  
  ann_precision_func <- function(data, index){ # creating a function for calculating the precision
    df <- data[index,]
    dum <- dummyVars(" ~ . - imp - participant_id - wave - fa_cat_end", data = df)
    dat <- cbind(data.frame(predict(dum, newdata = df)),fa_cat_end = df$fa_cat_end)
    mod <- ann_final
  ...
  ann_precision <- boot::boot(data = df_test, statistic = ann_precision_func, R = 1000) # applying the bootstrapping function on the testing dataset
  ann_precision_test[[i]] <- ann_precision$t # storing precision values
  
  ann_sensitivity_func <- function(data, index){ # creating a function for calculating the sensitivity
    df <- data[index,]
    dum <- dummyVars(" ~ . - imp - participant_id - wave - fa_cat_end", data = df)
    dat <- cbind(data.frame(predict(dum, newdata = df)),fa_cat_end = df$fa_cat_end)
    mod <- ann_final
  ...
  ann_sensitivity <- boot::boot(data = df_test, statistic = ann_sensitivity_func, R = 1000) # applying the bootstrapping function on the testing dataset
  ann_sensitivity_test[[i]] <- ann_sensitivity$t # storing sensitivity values
  
  ann_specificity_func <- function(data, index){ # creating a function for calculating the specificity
    df <- data[index,]
    dum <- dummyVars(" ~ . - imp - participant_id - wave - fa_cat_end", data = df)
    dat <- cbind(data.frame(predict(dum, newdata = df)),fa_cat_end = df$fa_cat_end)
    mod <- ann_final
  ...
  ann_specificity <- boot::boot(data = df_test, statistic = ann_specificity_func, R = 1000) # applying the bootstrapping function on the testing dataset
  ann_specificity_test[[i]] <- ann_specificity$t # storing specificity values
  
  ann_npv_func <- function(data, index){ # creating a function for calculating the specificity
    df <- data[index,]
    dum <- dummyVars(" ~ . - imp - participant_id - wave - fa_cat_end", data = df)
    dat <- cbind(data.frame(predict(dum, newdata = df)),fa_cat_end = df$fa_cat_end)
    mod <- ann_final
  ...
  ann_npv <- boot::boot(data = df_test, statistic = ann_npv_func, R = 1000) # applying the bootstrapping function on the testing dataset
  ann_npv_test[[i]] <- ann_npv$t # storing specificity values
  
  ann_cstat_func <- function(data, index){ # creating a function for calculating the C-statistic
    df <- data[index,]
    dum <- dummyVars(" ~ . - imp - participant_id - wave - fa_cat_end", data = df)
    dat <- cbind(data.frame(predict(dum, newdata = df)),fa_cat_end = df$fa_cat_end)
    mod <- ann_final
    pred <- predict(mod, dat, type = "raw") # applying the model to predict fatigue
    cstat <- as.list(rms::val.prob(unlist(pred), as.numeric(df$fa_cat_end), pl = FALSE, smooth = FALSE, statloc = FALSE))
    return(cstat$C)
  }
  ann_cstat <- boot::boot(data = df_test, statistic = ann_cstat_func, R = 1000) # applying the bootstrapping function on the testing dataset
  ann_cstat_test[[i]] <- ann_cstat$t # storing the function results from each bootstrapped instance
  
  ann_rsquared_func <- function(data, index){ # creating a function for calculating the R-squared
    df <- data[index,]
    dum <- dummyVars(" ~ . - imp - participant_id - wave - fa_cat_end", data = df)
    dat <- cbind(data.frame(predict(dum, newdata = df)),fa_cat_end = df$fa_cat_end)
    mod <- ann_final
  ...
  ann_rsquared <- boot::boot(data = df_test, statistic = ann_rsquared_func, R = 1000) # applying the bootstrapping function on the testing dataset
  ann_rsquared_test[[i]] <- ann_rsquared$t # storing the function results from each bootstrapped instance
  
  ann_slope_func <- function(data, index){ # creating a function for calculating the calibration slope
    df <- data[index,]
    dum <- dummyVars(" ~ . - imp - participant_id - wave - fa_cat_end", data = df)
    dat <- cbind(data.frame(predict(dum, newdata = df)),fa_cat_end = df$fa_cat_end)
    mod <- ann_final
  ...
  ann_slope <- boot::boot(data = df_test, statistic = ann_slope_func, R = 1000) # applying the bootstrapping function on the testing dataset
  ann_slope_test[[i]] <- ann_slope$t # storing the function results from each bootstrapped instance
  
  # Conducting statistical analysis on testing dataset
  ann_pred <- predict(ann_final, test, type = "raw") # applying the model to predict fatigue
  ann_pred_test[[i]] <- ann_pred # storing predicted values
  ann_roc_test[[i]] <- roc(test$fa_cat_end, as.numeric(ann_pred_test[[i]]))
  
  ann_prog <- ann_prog + 1 # tracking progress based on imputed dataset
  print(ann_prog)
}
ann_time <- Sys.time() - ann_start # for tracking computation time
ann_time

# Creating and printing ROC curves
ann_roc_plot <- plot(ann_roc_test[[1]], col = 1, lty = 2, main = "ANN model")
ann_roc_plot <- plot(ann_roc_test[[2]], col = 2, lty = 3, add = TRUE)
ann_roc_plot <- plot(ann_roc_test[[3]], col = 3, lty = 4, add = TRUE)
ann_roc_plot <- plot(ann_roc_test[[4]], col = 4, lty = 5, add = TRUE)
ann_roc_plot <- plot(ann_roc_test[[5]], col = 5, lty = 6, add = TRUE)
ann_roc_plot <- plot(ann_roc_test[[6]], col = 6, lty = 7, add = TRUE)
ann_roc_plot <- plot(ann_roc_test[[7]], col = 7, lty = 8, add = TRUE)
ann_roc_plot <- plot(ann_roc_test[[8]], col = 8, lty = 9, add = TRUE)
ann_roc_plot <- plot(ann_roc_test[[9]], col = 9, lty = 10, add = TRUE)
print(plot(ann_roc_test[[10]], col = 10, lty = 11, add = TRUE))

# Creating and printing calibration plot for internal validation
ann_pred_test_df <- data.frame(Predicted = unlist(ann_pred_test[[10]]), Observed = as.numeric(df_test$fa_cat_end))
ann_pred_test_df$Observed <- ifelse(ann_pred_test_df$Observed == 1, 0, 1)
#rms::val.prob(ann_pred_test_df$Predicted, as.numeric(df_test$fa_cat_end), pl = TRUE, smooth = FALSE, statloc = FALSE)
CalibrationCurves::val.prob.ci.2(ann_pred_test_df$Predicted, ann_pred_test_df$Observed, main = "ANN Model Calibration Curve")

# Creating and printing plots from bootstrapping analysis for internal validation
hist(unlist(ann_acc_test), main = "Histogram of Accuracy of ANN Model", xlab = "Accuracy")
hist(unlist(ann_balancedacc_test), main = "Histogram of Balanced Accuracy of ANN Model", xlab = "Balanced Accuracy")
hist(unlist(ann_precision_test), main = "Histogram of Precision of ANN Model", xlab = "Precision")
hist(unlist(ann_sensitivity_test), main = "Histogram of Sensitivity of ANN Model", xlab = "Sensitivity")
hist(unlist(ann_specificity_test), main = "Histogram of Specificity of ANN Model", xlab = "Specificity")
hist(unlist(ann_cstat_test), main = "Histogram of C-statistic for ANN Model", xlab = "C-statistic")
hist(unlist(ann_rsquared_test), main = "Histogram of R-squared for ANN Model", xlab = "R-squared")
hist(unlist(ann_slope_test), main = "Histogram of Calibration slope for ANN Model", xlab = "Calibration Slope")

# Printing statistical results for internal validation
result <- cat("Average Accuracy for ANN:", mean(unlist(ann_acc_test), na.rm = TRUE),
              "\nSD of Accuracy for ANN:", sd(unlist(ann_acc_test), na.rm = TRUE),
              "\nAverage Balanced Accuracy for ANN:", mean(unlist(ann_balancedacc_test), na.rm = TRUE),
              "\nSD of Balanced Accuracy for ANN:", sd(unlist(ann_balancedacc_test), na.rm = TRUE),
              "\nAverage Precision for ANN:", mean(unlist(ann_precision_test), na.rm = TRUE),
              "\nSD of Precision for ANN:", sd(unlist(ann_precision_test), na.rm = TRUE),
              "\nAverage Sensitivity for ANN:", mean(unlist(ann_sensitivity_test), na.rm = TRUE),
              "\nSD of Sensitivity for ANN:", sd(unlist(ann_sensitivity_test), na.rm = TRUE),
              "\nAverage Specificity for ANN:", mean(unlist(ann_specificity_test), na.rm = TRUE),
              "\nSD of Specificity for ANN:", sd(unlist(ann_specificity_test), na.rm = TRUE),
              "\nAverage C-statistic for ANN:", mean(unlist(ann_cstat_test), na.rm = TRUE),
              "\nSD of C-statistic for ANN:", sd(unlist(ann_cstat_test), na.rm = TRUE),
              "\nAverage R-squared for ANN:", mean(unlist(ann_rsquared_test), na.rm = TRUE),
              "\nSD of R-squared for ANN:", sd(unlist(ann_rsquared_test), na.rm = TRUE),
              "\nAverage Calibration Slope for ANN:", mean(unlist(ann_slope_test), na.rm = TRUE),
              "\nSD of Calibration Slope for ANN:", sd(unlist(ann_slope_test), na.rm = TRUE))
